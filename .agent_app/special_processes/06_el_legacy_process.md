...
|---|---|---|
| **Rules module (single source of truth)** | `review/Asset_dashboard_browser_EL/legacy_flow.py` | `get_building_process()`, `parse_legacy_ident()`, `parse_legacy_identity_text()`, `normalize_legacy_supply_from()`, `legacy_structured_from_raw()`, `compose_x_tag()`. Shared by the extraction API (importlib), the EL review app, and the SLD extractor. **Never fork its logic into a consumer.** |
...
| **Tests** | `test/test_legacy_flow.py` (core, ~199 tests) + `test_el_extraction_legacy_gate.py`, `test_el_upstream_fields_equipment_id.py`, `test_el_fed_from_backfill_legacy_guard.py`, `test_el_sync_legacy_gate.py` | Rule behavior, gate guards, and AST/behavioral regression locks. |

Legacy JSON envelope (in `Output_jason_api/`): `"process": "Legacy"`, `"el_legacy_rule_version": <n>`, `structured_data.label_text` (verbatim plate transcription — lets rules re-run without a new vision call and activates the ratings rules).
...
- **Golden examples:** QR `0000186130` → `DCC-1`, 208/120, 400, `Distribution - DCC-1`; QR `0000186128` → `PNL-UPS1`, Supply From = Fed From = `UPS`, 240/120, `Panel - PNL-UPS1`.
...
2. **Test:** add/extend cases in `test/test_legacy_flow.py` (golden-asset tests exist for 186128/186130) and run the full suite plus the four gate/guard test files.
...
4. **Deploy to VM:** copy the changed files (`legacy_flow.py`, API file, dictionary), then reload gunicorn by HUP-ing the master process (the one with ppid 1 — never `pgrep | head`).
5. **Regenerate:** the `ai_check.sh` cron only fires on `ai_status=0` counts, so run the extraction manually with env sourced: `set -a; . /home/developer/db_backend.env; set +a` first (an env-less shell falls back to SQLite and the gate fails closed — everything skips). For a single weak read: `EL_OVERWRITE_EXISTING_JSON=true ... --qr <code>`.
6. **Sync:** the review-app batch JSON→DB sync only runs on authenticated requests; run `sync_json_directory_to_db_el()` headless from the app venv with env set. Verify results in `sdi_dataset_EL`.
7. **Document:** dated entry in `attributes_changes.md` (newest at top), update the rules sections in `review_apps.rules.md` / `asset_extraction_api.rules.md` and this file, then sync all four doc locations: canonical → `.agent_app/` mirrors → VM (`/home/developer/Markdowns_documentation/` + `/home/developer/.agent_app/`) → SecondBrain vault.
