...
- asset disposal (`disposed_assets`) and its cross-app hide/guard rules
...
- **Disposed Assets tool** added to the Dashboard's Operations & Monitoring section (2026-08-11): new PostgreSQL table `disposed_assets` (owner-run migration `scripts/migrations/2026-08-11_disposed_assets.sql`; partial unique index for one open disposal per QR, no FK to `QR_codes` so the archive survives purge, app role `REVOKE DELETE`). Disposal archives a full snapshot, deletes the curated `sdi_dataset` / `sdi_dataset_EL` row, normalizes `ai_status`, and audits in-transaction (`app_name="dashboard_disposed"`) — all in one transaction with **no file writes**. Gate: only **not-approved**, unpackaged, not-already-disposed assets, and a reason is always required (`Decommissioned` / `Duplicated` / `User Request`). Because the review dashboards list from `Output_jason_api/` rather than the curated tables, all three reviewers subtract the disposed set in `get_filtered_data_and_counts` and refuse `toggle_approved` (409) — deleting the SDI row alone would not hide the asset. The capture app flags it in `/api/check-qr` and refuses `/submit` before writing files; the three AI interpreters add it to `_load_qrs_to_ignore` (survives `FORCE_REPROCESS`) and keep it out of the `_load_ai_processed_qrs` stale-reset. Admin-only dispose/restore via new RBAC key `operations` / `disposed_assets`; reads need only login. Service layer `Dashboard/disposed_assets_service.py`. See `rules/dashboard.rules.md` → "Disposed Assets Rules".

- **Disposed Assets follow-ups** (2026-08-12): (a) reason code **`Wrong Asset`** added — `DISPOSAL_REASONS` and `chk_disposed_reason` widened together (migration `scripts/migrations/2026-08-12_disposed_reason_wrong_asset.sql`; the dropdown and the register filter both read the list from the API, so no template change was needed). (b) The mutation gate dropped from **`admin` to `editor`** on `operations` / `disposed_assets`: the User Admin matrix offers only `none`/`viewer`/`editor` and `api_admin_permissions_put` rejects any other level, so the `admin` requirement was ungrantable through the platform's own UI and blocked every user — `has_permission()` resolves an exact section/item grant and never treats the `is_admin` flag as a wildcard. The blocked-user hint now names the grant to ask for. (c) The disposal **confirmation dialog dropped the type-the-QR step**: it now asks `Dispose asset <QR>?` over the same summary and posts on `OK` — disposal is Restore-reversible and already gated by the eligibility checklist, so the retype (borrowed from the dictionary delete dialog) bought nothing. (d) A **`Disposed Assets` item was added to the shared shell sidebar** under `Operations` (key `disposed`, icon `archiveX`, href `…/#disposed`), propagated to all five byte-identical `shell.js` copies with the `?v=` cache-bust bumped to `20260812-1` in every `_shell.html`; the Dashboard, the three reviewers and SDI Process were reloaded so the new version string is served. (e) **Disposed assets left the Overview numbers**: `charts/approval.py::_scan_capture_universe()` and `charts/flow_quantity_chart.py::_review_state_counts()` / `_flow_counts()` subtract the active-disposal set, so the KPI section (bars, Manual Entry slice, chip, integrity/"unsynced" warning) and Pipeline Overview (review cards, Total QR pill, SDI flow steps) all ignore retired assets — on production this took the false unsynced count from 2 to 0 and Total QR from 1,602 to 1,600. Both modules must subtract the same set or `integrity_snapshot()`'s reconciliation identity breaks. The JSON-driven Analytics / Performance Analysis charts still include disposed QRs (documented, per-chart decision pending). Docs touched: `rules/dashboard.rules.md`, `workflows/04_dashboard_ops.md`, `02_SYSTEM_MAP.md`, `docs_index.md`, `special_processes/04_database_topography.md`, `Dashboard/.agent/RULES.md`.
