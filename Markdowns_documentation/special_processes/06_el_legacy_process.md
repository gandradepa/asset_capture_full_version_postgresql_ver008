...
- **Power Rating is transformer-only.** Gated by `is_legacy_transformer()` (tag or Equipment ID matching `TX`, tolerating the drifted `T-X-` spelling, or a `Transformer` equipment type). A panel whose lamacoid quotes its upstream feeder — DCC-1's real plate prints `THROUGH TRANS. "T1" 112.5 K.V.A.` — must never inherit that rating; keeping nameplate text in its own field is what guarantees this.
