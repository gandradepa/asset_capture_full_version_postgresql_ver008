...
## JSON Sync Retry Guard (ME / BF / EL) — 2026-08-05

The batch `sync_json_directory_to_db*` functions decide what to process by comparing each JSON's mtime against `processed_json.log`. A file marked processed is therefore **never revisited** unless it is edited again, so marking a file processed without having written its curated row loses the record permanently.

- **A JSON may only be marked processed after its curated row was actually written.** The upsert helpers return a boolean (`_db_upsert_sdi_dataset` in ME, `upsert_sdi_dataset` in BF): `False` on unreachable DB, missing table/column, unbuildable payload, or exception; `True` only after `conn.commit()`. The sync loop must `continue` (leaving the file unmarked) whenever the write returns `False`.
- **Every reviewer's sync must bail out of the whole pass when the DB is unreachable** (`if not _connectable(): return`, before any file is marked). EL had this guard from the start; ME and BF got it on 2026-08-05.
- The deliberate exception stays: genuinely sparse `structured_data` **is** marked processed (with a warning) to prevent endless replay — that is a permanent data condition, not a transient failure.
- **Blank foreign-key values must be written as NULL, never `''`.** `sdi_dataset` carries `fk_sdi_mainasset` → `main_asset_description(main_asset)`, and that reference table has no empty-string row, so `Main Asset = ''` makes PostgreSQL reject the entire INSERT (`Key (Main Asset)=() is not present`). NULL is accepted (65 curated rows use it). ME routes every curated write through `_column_value_for_write()` / `NULL_WHEN_BLANK_COLUMNS` for exactly this reason; BF is unaffected because it writes the constant `Domestic Water Distribution Equipment`, which is a valid reference. Verify against the live constraint set before adding a column to that set.
- **Why (incident 2026-07-29, root cause confirmed 2026-08-06):** 22 Mechanical QRs (17 in building `023-1`, plus 020, 166, 213, 353, 836, 904) were captured and extracted successfully but never reached `sdi_dataset`, surfacing as Dashboard KPI 1,540 vs pipeline 1,577. The cause was **deterministic, not a transient outage**: none of those assets matched the dictionary, so ME wrote `Main Asset = ''`, `fk_sdi_mainasset` rejected the INSERT, `_db_insert_row_helper` swallowed the `IntegrityError`, the upsert still reported success, and the JSON was marked processed — permanently. Any Mechanical asset without a dictionary match was affected. Regression tests: `test/test_json_sync_retry_guard.py` (+ `test/_json_sync_guard_probe.py`), whose fixture reproduces the PK width mismatch and the FK shape, and which were verified by mutating each half of the fix independently.
- Healing a stranded record means clearing its `processed_json.log` entry (back the log up first) and letting the reviewer's own sync re-write it — never hand-written SQL, so dictionary rules, `Col_process` registration, and audit rows all still apply.

## Sync Outcome Persistence (ME / BF) — 2026-08-06

The retry guard above prevents loss but left no trace an operator could find: a `FAILED` / `CONFLICT` / sparse outcome produced a bare `print()` to the gunicorn journal (no `logging` handler, nothing in `logs/`), and `audit_trail` was written **only** on success. `json_files` has no status column, and the processed bookkeeping is a flat `processed_json.log`. So after the 2026-07-29 loss there was nothing to query.

- **Every non-OK sync outcome writes an `audit_trail` row** via `_record_sync_warning(qr, building, filename, status, detail)` in each reviewer. Statuses: `SDI_WRITE_FAILED`, `SDI_WRITE_CONFLICT`, `SPARSE_SKIPPED`. The Dashboard's **User Activity** view surfaces them with no new UI — filter `source = system` (`modified_by = system:json-sync`).
- **`op_type` must be `'UPDATE'`.** `audit_trail` carries `CHECK (op_type IN ('INSERT','UPDATE','DELETE'))` (`scripts/migrate_create_audit_trail.py:32`), so the real signal rides in a synthetic `sync_status` field (`field_name = 'sync_status'`, `new_value = "<STATUS>: <detail>"`). Do **not** invent a new `op_type` without also widening that CHECK and `audit/logger._VALID_OPS`.
- **Always use a fresh connection.** The caller may hold one whose transaction a constraint error already aborted, where under PostgreSQL any further write is silently discarded at COMMIT (the mechanism behind the original incident).
- **Best-effort, never fatal.** The helper swallows its own errors: losing a warning row must never cost the sync. Probe scenario G5 asserts the sync still completes with `audit_trail` dropped entirely.
- **Deduped per process** by `(filename, status)` in `_SYNC_WARNED`, because `SDI_WRITE_FAILED` retries on every request pass and one row per request would bury the trail it exists to provide. A later `SDI_WRITE_OK` calls `_clear_sync_warning(filename)` so a regression alerts again. Probe G3 proves three failing passes write exactly **one** row, using the real constraint-rejection path (not a broken connection — that would also break the audit write and make the assertion vacuous).
- **The sparse-skip still marks processed**, deliberately: replaying placeholder JSONs on every request is the storm the CONFLICT path avoids. Its distinct `SPARSE_SKIPPED` status is what makes the skip auditable, and `scripts/audit_capture_vs_curated.py` catches any sparse skip that actually strands a captured QR.
...
- ME, BF, and EL listing tables expose `QR_codes.capture_notes` as a display-only `Notes` icon immediately after `Capture Date`: green when a nonblank note exists and muted/faint when empty. The per-asset **ME and EL** review pages show the same QR-level note in a read-only box beside the thumbnail strip — on EL this card lives in the shared visual panel, so it renders for **both** form variants (General and Distribution); BF currently surfaces the note only on its printed sheet. The card carries no `name` attribute and no input, so it never enters the review POST payload. This is not posted back to JSON, SDI, Planon, or audit tables.
...
- **Simplex `UN-` tag rule (2026-08-08)** — when extraction captures a Manufacturer containing "Simplex" (any casing, incl. SimplexGrinnell), `_apply_el_simplex_tag_rule` **always overwrites** `UBC Asset Tag` with `UN-<Model>` (whitespace-stripped, uppercased; bare `UN-` while no model is read), in both standard and legacy flows, with confidence = min of the source-field confidences (a bare `UN-` is capped below `MANUAL_REVIEW_MIN_CONFIDENCE` so the placeholder still flags for review). Simplex fire-alarm panels carry no UBC lamacoid, so any read tag is junk. When it fires the rule also re-syncs tag-derived companions: `Branch Panel` cleared in both flows, and in the legacy flow `Equipment ID` follows the new tag while `Equipment Type`/`Power Type` clear and `Description` recomposes as `SIMPLEX - <tag>` (feeder fields are left alone). Do NOT add `UN` to the API `Config.ABBREVIATIONS` (it would mangle raw tags starting with those letters); instead the **standard flow re-applies the rule after its post-loop `_resolve_el_asset_tag` + `_apply_tag_formatting` step** — without that re-application the tag would ship as `PNL-UN-<Model>` and the synthesized tag would leak into `Branch Panel`. The dictionary entry `UN-|EL` (description SIMPLEX) then derives Asset Group `Fire Alarm Annunciator Panels` (a General group → nameplate form variant), Attribute `FireAlarmPanel`, and Main Asset — group/attribute changes for these assets are dictionary data edits, not code.
...
| 2 | `Voltage Rating` (+`UoM`) / `Volts` | tag voltage code `2`/`6` via dictionary | ✅ ratings line (`120/208V`) | Parse from label text, NOT from ident. Store bare, higher-first (`208/120`), UoM `VLT`. Skip if `volts_manual_override`. **Unit placement (2026-08-05, Siemens EQ loadcentres):** value-then-unit requires the `V` on the same line (a `\s*` gap let `Amps 225` + newline + `Volts` header parse as a fabricated 225 V), and unit-first columns (`Volts AC/CA 120/240`, `Amps 225`) are accepted with same-line, letterless windows — tap-table headers (`Pos. Volts` / lettered rows) and breaker interrupting ratings (`RMS SYM AMPS 10,000`) can never match. |
