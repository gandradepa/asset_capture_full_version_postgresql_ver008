...
- EL AI extraction reads the four nameplate fields from the `-0` Asset Plate for **all** EL assets (standard + legacy flows) — General cannot be classified before extraction. They are not in any extraction scoring/flag tuple, and their confidences are projected **only when non-blank** so pre-existing JSONs are never mass-flagged stale (no `EXTRACTION_RULE_VERSION` / `EL_LEGACY_RULE_VERSION` bump — a bump re-extracts the whole corpus). `normalize_manufacturer` is a BF whitelist: the EL wiring falls back to a light cleanup (`_clean_raw_manufacturer`) and must never blank a non-empty read.
