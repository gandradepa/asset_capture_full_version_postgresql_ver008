...
- Triggering AI extraction on uploaded nameplate photos
- Debugging a specific, failing asset extraction (via `--debug` and `--qr`)

## Do not use this skill when

- The asset is already approved or flagged in `sdi_dataset`

## Instructions

Follow the Extraction Pipeline sequence: discovery, OCR preprocessing, LLM extraction, and DB synchronization.
...
3. Preprocess   â†’ CLAHE + adaptive thresholding + Digital Rotation (ver04)
4. OCR          â†’ Run Tesseract with multiple rotations + PSM modes
5. LLM Call     â†’ Send images + OCR context + Hallucination Guardrails
6. Validate     â†’ Local normalizers (ver04) + Shared validators
