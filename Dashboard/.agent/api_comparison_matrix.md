...
| **OCR Calls/Image** | Variable (light: on-demand, full: ~9 variants) | **4** (Extreme Optimized) | Variable (context injection) |
| **Context Injection** | OCR text â†’ LLM prompt as hint | OCR text â†’ LLM prompt | OCR text â†’ LLM prompt |
| **Rotation Strategy** | 0Â°, 90Â° CW, 90Â° CCW | 0Â°, 90Â° CW, 90Â° CCW | 0Â° primary |
...
| Logic | ME | BF | EL |
| :--- | :--- | :--- | :--- |
| **Image Grouping** | Regex (Seq 0, 1, 3) | Regex (Seq **0, 1** only) | Regex (Seq 0, 1, 2) |
| **Special Rules** | FCU MI/M1/ML disambiguation, tag-vs-model guardrail, manufacturer OCR recovery | Diameter suffix check (`"`), shared manufacturer validation | **PNL-** prefix enforcement, panel tag grammar |
