...
- **Simplex `UN-` tag rule (2026-08-08):** extraction sets `UBC Asset Tag = UN-<Model>` (always overriding the read tag) whenever Manufacturer contains "Simplex" — these fire-alarm panels have no UBC lamacoid. Dictionary entry `UN-|EL` derives Fire Alarm Annunciator Panels / FireAlarmPanel downstream. Never add `UN` to `Config.ABBREVIATIONS`.
