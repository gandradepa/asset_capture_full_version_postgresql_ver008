...
- **General vs Distribution review form (2026-08-07):** `review.html` renders one of two server-selected variants via `_el_form_variant(asset_group)` (never via `base_route`). General assets get an ME-style nameplate form (Manufacturer, Model, Serial Number, Year + Installation Date; Classification card with UBC Asset Tag / Asset Group / readonly Attribute / Main Asset; Location kept; no TSBC, no electrical tech cards) with Equipment ID/Type/Power Type still derived+stored server-side. `sdi_dataset_EL` gained the four nameplate columns (blank on Distribution rows; migration `2026-08-07_sdi_dataset_el_nameplate_columns.sql`). EL extraction reads the four fields from `-0` (preferred; since 2026-08-08 `-1` is an allowed fallback only when it clearly shows a manufacturer plate — facility-made text is never a source) for all EL assets with **no rule-version bump** — confidences project only when non-blank (bumping the version or projecting blanks mass-re-extracts the corpus). General review scoring = UBC Asset Tag + the four nameplate fields. SDI: `build_sdi_dataset()` renames EL `Serial Number`→`Serial` so all four reach Planon (Make/Model/Serial Number/Date Of Manufacture). Details: `Markdowns_documentation/rules/review_apps.rules.md` → "EL Review Form Variants".
- **Fire-alarm panel tag rules (2026-08-08, family 2026-08-10):** the API's `fire_alarm_control` class (subclasses `Simplex`, `Autocall`; Simplex wins) keys off the extracted Manufacturer — these fire-alarm panels have no UBC lamacoid, so the read tag is always overridden. Simplex → `UBC Asset Tag = UN-<Model>`; Autocall → tag = the model number itself (e.g. `4100ES`) with Manufacturer canonicalized to `AUTOCALL`. Dictionary entries `UN-|EL` (→ Fire Alarm Control Panels) and `4100|EL` (→ Fire Alarm Annunciator Panels) derive group/attribute downstream. Never add `UN` or `4100` to `Config.ABBREVIATIONS`.
- **Capacity pair + General listing columns (2026-08-07 follow-up):** the General form's Identity card also captures the OPTIONAL `Capacity` + `Capacity (UoM)` pair (bare value, unit as printed — no fixed code list) — stored on `sdi_dataset_EL` **and** the package tables (migration `2026-08-07_el_capacity_columns.sql`), General rows only, deliberately excluded from review scoring/checklist (`EL_CAPACITY_FIELDS` stays out of the review app's `EL_NAMEPLATE_FIELDS`; the API-side tuple DOES include it for the gate exclusions and non-blank-only projection). Exported via `PACKAGE_ONLY_COLS` to Planon `Capacity`/`Capacity UoM` (`ATTRIBUTE_SETS.py` lists both under `Electrical`). The General listing (`/review-all`) hides Supply From/Amperage Rating/Volts/Location (`generalHiddenColumns`); the Distribution listing keeps its existing hidden set (Amperage/Volts/Location).
